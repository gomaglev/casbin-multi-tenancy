...
func (a *Role) Query(ctx context.Context, params schema.RoleQueryParam, opts ...schema.RoleQueryOptions) (*schema.RoleQueryResult, error) {
	...
	if v := params.TenantID; v != "" && v != "wetrue" {
		db = db.Where("id <> ?", config.C.TenantOwnerRole.ID)
	}
...
// Get 查询指定数据
func (a *Role) GetOwnerRoleID(ctx context.Context, opts ...schema.RoleQueryOptions) (*schema.Role, error) {
	var role entity.Role
	ok, err := FindOne(ctx, entity.GetRoleDB(ctx, a.DB).Where("type=9"), &role)
	if err != nil {
		return nil, errors.WithStack(err)
	} else if !ok {
		return nil, nil
