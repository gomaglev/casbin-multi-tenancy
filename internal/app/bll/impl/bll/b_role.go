...
// InitData 初始化菜单数据
func (a *Role) InitData(ctx context.Context, dataFile string) (*schema.IDResult, error) {
	role, err := a.RoleModel.GetOwnerRoleID(ctx)
	if err == nil && role != nil {
		if role.ID != "" {
			return schema.NewIDResult(role.ID), err
		}
	}

	id := iutil.NewID()
	err = a.RoleModel.Create(ctx, schema.Role{
		ID:       id,
