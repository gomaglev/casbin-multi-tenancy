...
const (
	...
	TenantIDKey      = prefix + "/tenant-id"
	IsAdminIDKey     = prefix + "/is-admin"
...
func IsAdmin(c *gin.Context) bool {
	return c.GetBool(IsAdminIDKey)
}

// SetIsAdmin 设定是否管理用户
func SetIsAdmin(c *gin.Context, isAdmin bool) {
	c.Set(IsAdminIDKey, isAdmin)
}
...
func ResError(c *gin.Context, err error, status ...int) {
	...
	if err != nil {
		if e, ok := err.(*errors.ResponseError); ok {
			...
			res = errors.UnWrapResponse(errors.Wrap500Response(err, "ErrServerError"))
		}
	} else {
